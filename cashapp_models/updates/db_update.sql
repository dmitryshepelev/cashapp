...
CREATE TABLE "cashapp_models_finance" ("id" serial NOT NULL PRIMARY KEY, "guid" varchar(40) NOT NULL UNIQUE, "is_exist" boolean NOT NULL, "name" varchar(30) NULL, "is_locked" boolean NOT NULL, "currency_id" varchar(3) NOT NULL);
CREATE TABLE "cashapp_models_financeregister" ("id" serial NOT NULL PRIMARY KEY, "guid" varchar(40) NOT NULL UNIQUE, "is_exist" boolean NOT NULL, "date" timestamp with time zone NOT NULL, "balance" numeric(17, 2) NOT NULL, "finance_id" varchar(40) NOT NULL);
CREATE TABLE "cashapp_models_financetype" ("id" serial NOT NULL PRIMARY KEY, "name" varchar(10) NOT NULL UNIQUE);
ALTER TABLE "cashapp_models_finance" ADD COLUMN "type_id" varchar(10) NOT NULL;
...
ALTER TABLE "cashapp_models_finance" ADD CONSTRAINT "cashapp_mod_currency_id_e07f46f_fk_cashapp_models_currency_code" FOREIGN KEY ("currency_id") REFERENCES "cashapp_models_currency" ("code") DEFERRABLE INITIALLY DEFERRED;
...
CREATE INDEX "cashapp_models_finance_currency_id_e07f46f_like" ON "cashapp_models_finance" ("currency_id" varchar_pattern_ops);
...
CREATE INDEX "cashapp_models_financetype_name_37aaf4b2_like" ON "cashapp_models_financetype" ("name" varchar_pattern_ops);
CREATE INDEX "cashapp_models_finance_94757cae" ON "cashapp_models_finance" ("type_id");
ALTER TABLE "cashapp_models_finance" ADD CONSTRAINT "cashapp_mod_type_id_68645f30_fk_cashapp_models_financetype_name" FOREIGN KEY ("type_id") REFERENCES "cashapp_models_financetype" ("name") DEFERRABLE INITIALLY DEFERRED;
